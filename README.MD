...
<sub>Responsive design of the website</sub><br><br>
...
#### Imagery
- There are five different hero background images implemented on every page and will be randomly loaded. The background images are all street scenes from Tokyo made by the website owner in the year 2019. The colours are adjusted in such a way it won't distract the information in the foreground.
...
#### Wireframes
Wireframes belonging to the website's project have been made in [Adobe XD](https://www.adobe.com/products/xd.html).
...
### About Page
The 'About' page gives information to the visitor about the history and background of the Citypop genre. Also embedded are two YouTube videos about the history of Citypop and a video of Western tourists buying Citypop LP records at a local used record store in the Shibuya district of Tokyo.


### Add Song Page
On the 'Add song' page, only a logged-in user can add songs to the Citypop Central database. The user needs to add the necessary song information into a form. On top of every data field, some extra information is given to the user to add the correct data information. Regular expression is introduced to force the user to add correct data to the database. Because it is not possible to store images in the database, all album cover images have to be loaded from the [discogs.com](https://www.discogs.com/) website by adding the link into the image data field. A typical link to one of these album covers from the discogs website looks like this: https://img.discogs.com/mEAMMAzV336RAr0O0FjH41Q7oY4=/fit-in/600x600/filters:strip_icc():format(jpeg):mode_rgb():quality(90)/discogs-images/R-4777778-1618165602-9892.jpeg.jpg. In case no album cover image is available, the album image data field needs to stay empty. The website automatically adds a 'no-cover' image to the form when leaving the album image data field and will pop up on the screen. Adding the song link from the Youtube website is done by adding the video link ID. For every YouTube song, this ID can be found in the address bar when playing a specific song on the YouTube website. For example, the hyperlink in the address bar looks like this: https://www.youtube.com/watch?v=pE2D3LWADFg. Here 'pE2D3LWADFg' is the video link ID! Copy this and paste it into the YouTube Link data field.


### Edit Song Page
The 'Edit song' page looks and works the same as the 'Add song' page and functions almost the same. Instead of adding a song to the database, a song can be edited here. At the bottom below the 'Edit song' button, a 'Delete song' section has been introduced. By clicking the hyperlink 'Click here to delete this song' another hyperlink will pop up in a red colour 'Confirm to delete this song'. By clicking the second hyperlink, the song will be deleted from the database. Only the user who originally added the song to the database is able to edit the song, with an exception for the administrator or 'admin' of the website.
...
On the 'My Songs' page, a summary of all uploaded songs by the user is indicated. In case logged in as the administrator 'admin' of the website, this page (All Songs) will summarize ALL songs in the database. Songs are alphabetically sorted by the artist's name and then song name. By clicking on one of the songs, the user will be re-directed to the 'Edit song' page. Here the user or admin can change the song's data or even delete the song.
...
### User Info Page
The 'User Information' page gives information about a user to other users or visitors. The email address indicated on the 'User Information' page is only visible to logged-in users. A user can turn off the visibility of their email to other users on the 'User Information' page by disabling it on their 'Profile' page. To access the user information page, click on the expand link (?) of a song to see which user uploaded the song (only when you are logged in as a user). By clicking on the 'Uploaded By' username the user information page will pop up (blank page).


### Users Page (for admin only)
Only the administrator 'admin' of the website has access to the 'Users' page. Here the administrator sees a summary of all users and can delete unwanted users from the database by clicking the delete icon 'Garbage Bin'. A second red coloured 'Garbage Bin' icon will show up and by clicking this icon, the specific user will be deleted from the website and database and will be no longer able to enter his account.


### YouTube Page
By clicking the Artist Name or Song Name at the top of every search record on the 'Results' page, the visitor will be automatically redirected (blank page) to the website's 'YouTube' page of the song. On this page, the YouTube video of the song has been embedded and the video will automatically play when the page is loaded.
...
- [Git](https://git-scm.com/) - Git was used for version control and code change tracking of the website project. The terminal in Gitpod was used to commit and push the project to the Github repository.
- [Github](https://github.com/) - The project code has been stored on the Github website.
- [GitPod](https://www.gitpod.io/) - GitPod was used for creating the code, testing the website, committing, and pushing the code to the Github repository.
...
- [Photopea.com](https://www.photopea.com/) - The Photopea website was used for creating the README file images and favicon image. 
- [Flask](https://en.wikipedia.org/wiki/Flask_(web_framework)) - Flask is a micro web framework written in Python used for interaction between the front-end and back-end of the website.
...
- Check the latest uploaded song links function and open a new screen. **(checked)**
...
- Check whether the link redirects to the homepage. **(checked)**
...
- Check whether the album image loads correctly. **(checked)**
...
- Check whether the album image loads correctly. **(checked)**
- Check the regular expression. **(checked)**
- Check whether the admin can edit all songs. **(checked)**
- Check whether the admin can delete all songs. **(checked)**
- Check whether other users cannot edit songs via the address bar. **(checked)**
- Check whether visitors cannot edit songs via the address bar. **(checked)**
...
- Check the responsiveness of the page. **(checked)**
- Check whether Artist Name and Song Name are properly indicated. **(checked)**
...
- Check regular expression. **(checked)**
- Check whether other users have no access to the profile page by using the address bar. **(checked)** 
- Check whether visitors have no access to profile the page by using the address bar. **(checked)**
...
<img src="static/readme-images/icons.jpg" alt="Youtube, Delete, Edit, and Expand more information Icons"><br>
<sub>Youtube, Delete, Edit, and Expand more information Icons</sub>


#### Settings Page (for admin only)
- Check the responsiveness of the page. **(checked)**
- Check whether only the admin has access to this page. **(checked)**
...
- Check whether it is possible to delete a release year from the database. **(checked)**
- Check whether it is possible to add a release year from the database. **(checked)**
- Check regular expression. **(checked)**
...
- Check e-mail address is hidden for users and visitors when turned off on the 'Profile' page. **(checked)**
- Check non-existing user return message when the user does not exist. **(checked)**
...
- Check the responsiveness of the page. **(checked)**
- Check whether only the admin has access to this page. **(checked)**
- Check whether all users show up from the database. **(checked)**
...
<sub>Flash Messages</sub>


### Usability Testing
- Family and friends were asked to test the website and to navigate through all website pages. Buttons, links, and forms were tested by them.
...
No serious errors were discovered.
...
Warnings:  
The validator does not recognize CSS variables.  
...
1. As a visitor of the website, I want to understand what the website's purpose is.
   ...
   * The 'Search songs in our database' search bar and button on the 'Index' page tells it is possible to search for songs on the website.
   * Suggestions for keywords are added above the search bar to help the visitor use the correct keywords.
   ...
   * On the 'Register' page, it tells visitors they can add their songs into the database when they register.
...
2. As a visitor of the website, I want to navigate easily throughout the website, so I don't get frustrated not knowing where to click.
   ...
   * The website is easily usable for visitors who are only searching for songs. All links and buttons are foreseen with a title explaining the purpose which can be accessed when hovering.
...
6. As a visitor of the website, I might want to contact the website owner, so I can ask a question or give some feedback.
   * On the bottom of the following pages: 'Index,' 'About' and 'Youtube' pages, hyperlinks to the website's social media accounts are indicated. By clicking on one of the links, the visitor will be redirected to these social media accounts. Here the visitor can send direct messages to the website owner.
...
<sub>Navigation Bar Logged in User</sub>
...
3. As an administrator of the website, I want to know when songs were updated for the last time.
   * Songs will get an extra 'Edit Date' data field when the song was edited after it was added to the database. Click the expand more information (?) button for every search record to see this field below the 'Upload Date' field.
4. As an administrator of the website, I want to be able to delete unwanted users.
...
<a id="bugs"></a>
## Known Bugs
* No significant bugs were found on the website by the website creator.
...
* Regular expression check for YouTube video ID and image hyperlink for the album cover image is almost impossible to check, other solutions need to be found in the future.
...
* The Python code in the app.py file exceeds more than 1000 lines, this gives a warning in the GitHub terminal.
...
* Users can delete their profile directly via the address bar without defensive programming '/delete-profile', this is only possible when a user knows about this possibility, although it's better to fix this in the future.
...
* Songs don't autoplay on phone and iPad.
...
#### Images
- The website logo and favicon were created by the website creator.
...
### Acknowledgments
- Thanks to my personal mentor Victor Miclovich for answering my questions and giving feedback on this project.
